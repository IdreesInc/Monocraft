...
def parseArgs():
    ...
    if ret.all:
        ret.black = ret.bold = ret.semibold = ret.light = \
            ret.extralight = ret.italic = True
    return ret


def generateFont(*, black=False, bold=False, semibold=False, light=False,
                 extralight=False, italic=False, output_ttc=False, **kw):
    fontList = [
        fontforge.font(),
    ...
    for ligature in ligatures:
        ...
        createChar(fontList, -1, ligature["name"], image,
                   width=PIXEL_SIZE * len(ligature["sequence"]) * 6, **kw)
        for font in fontList:
            if font is None:
                continue
            font[ligature["name"]].addPosSub("ligatures-subtable", tuple(map(
